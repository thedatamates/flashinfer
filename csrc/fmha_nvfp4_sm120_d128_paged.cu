...
namespace d128 = attention::blackwell::sm120_nvfp4::d128;
using sm120_nvfp4_paged::PagedKernelConfig;
using sm120_nvfp4_paged::PagedParams;

cudaError_t Sm120Nvfp4D128RunPagedRaw(
  ...
  return d128::sm120_nvfp4_qkv_online_register_q_splitkv_full_grid_raw<
      1, true, SM120_NVFP4_CAUSAL, SM120_NVFP4_USE_SLIDING_WINDOW,
      ...
      sliding_window, logits_soft_cap, q_rows, d128::kHeadDim, kv_len, stream,
      paged_params, qo_indptr, kv_lens, batch_size, q_tiles_per_sequence,
      skip_internal_combine);
}

PagedKernelConfig Sm120Nvfp4D128PagedKernelConfig() {
  return {d128::kHeadDim,
          d128::kCutlassTileM,
          1,
          SM120_NVFP4_CAUSAL,
...
void SM120Nvfp4FmhaRunPagedBatch(
  ...
  const auto kernel = Sm120Nvfp4D128PagedKernelConfig();
  sm120_nvfp4_paged::RunPagedBatchImpl(
      ...
      output_group_span, v_cache_uses_pv_layout,
      normal_v_scales_are_trtllm_interleaved, kv_layout_hnd);
}
