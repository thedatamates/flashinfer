/*
 * Direct SM120 FP4 MMA probe for A-fragment K ordering.
 *
 * B is all ones. A is a function of K only and is constant across M. Every
 * output element must therefore be identical if A fragment packing matches the
...
__device__ __forceinline__ float e2m1_k_value(uint32_t k) {
  constexpr float vals[7] = {0.5f, 1.f, 1.5f, 2.f, 3.f, 4.f, 6.f};
  return vals[k % 7];
}

__device__ __forceinline__ uint32_t make_a_frag_row_ramp(uint32_t reg) {
  const uint32_t lane = threadIdx.x & 31;
  float vals[8];
#pragma unroll
  for (uint32_t i = 0; i < 8; ++i) {
    const uint32_t value_idx = i * 4 + reg;
    const uint32_t k = ((lane >> 2) & 0x7u) + 16 * (value_idx & 0x3u) +
                       8 * ((value_idx >> 3) & 0x1u);
    vals[i] = e2m1_k_value(k);
  }
  return flashinfer::mma::float8_to_e2m1x8(vals[0], vals[1], vals[2], vals[3], vals[4],
                                           vals[5], vals[6], vals[7]);
}

__global__ void a_row_ramp_probe_kernel(float* out) {
#if defined(__CUDA_ARCH__) && (__CUDA_ARCH__ == 1200 || __CUDA_ARCH__ == 1210)
  uint32_t a[4] = {
      make_a_frag_row_ramp(0),
      make_a_frag_row_ramp(1),
      make_a_frag_row_ramp(2),
      make_a_frag_row_ramp(3),
  };
...
int main() {
  ...
  a_row_ramp_probe_kernel<<<1, 32>>>(out);
