/*
 * Probe the full production BMM1 D-tile loop for persistent-Q + paged-K NVFP4.
 */
...
#pragma unroll
      for (int elem = 0; elem < 8; ++elem) {
        ...
        out[row * CtaTile::N + col] = acc[mi][ni].elt(elem);
      }
    }
  }
#endif
...
int main() {
  ...
  probe_kernel<<<1, CtaTile::THREADS_PER_CTA,
                 KernelTraits::BYTES_PER_SMEM_Q + SmemK::BYTES_PER_TILE>>>(params, out);
  ...
  for (int q_token = 0; q_token < q_len; ++q_token) {
    ...
    for (int q_head = 0; q_head < num_q_heads; ++q_head) {
      ...
      bool const is_bad = target_val < 100.f || other > 1e-3f;
      if (is_bad) {
        std::printf("q_token %d head %d row %02d target %02d val %.3f other %.3f at %02d BAD\n",
