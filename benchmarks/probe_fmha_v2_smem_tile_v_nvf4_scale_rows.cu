/*
 * Probe Smem_tile_v scale loading for all logical K rows.
 */
...
__global__ void probe_kernel(float* out) {
  ...
  int const active_group = active_row / Traits::NVFP4_SCALE_VEC_SIZE;
  ...
  for (int idx = threadIdx.x; idx < CtaTile::N * (CtaTile::K / Traits::NVFP4_SCALE_VEC_SIZE);
    ...
    uint8_t const scale = scale_group == active_group
                              ? fmha::float_to_e4m3_byte(72.f)
                              : fmha::float_to_e4m3_byte(0.f);
    smem_v.store_v_scale(col, scale_group, scale);
  }
  __syncthreads();
...
int main() {
  ...
  for (int row = 0; row < 64; ++row) {
    ...
    bool const ok = mn == 432.f && mx == 432.f;
    bad += ok ? 0 : 1;
