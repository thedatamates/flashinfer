...
__global__ void PersistentVariableLengthMergeStatesKernel(
  ...
  constexpr uint32_t vec_bits = sizeof(DTypeIn) * vec_size * 8;
  constexpr uint32_t head_dim = vec_size * bdx;
  const uint32_t out_stride_n = o_stride_n == 0 ? num_heads * head_dim : o_stride_n;
  const uint32_t out_stride_h = o_stride_h == 0 ? head_dim : o_stride_h;
  extern __shared__ uint8_t smem[];
  DTypeIn* v_smem = (DTypeIn*)smem;
  float* s_smem = (float*)(smem + num_smem_stages * bdy * head_dim * sizeof(DTypeIn));
...
#pragma unroll 1
  for (uint32_t i = cta_id; i < seq_len * num_heads; i += num_ctas) {
    ...
    if (num_index_sets == 0) {
      vec_t<DTypeO, vec_size> v;
      v.fill(DTypeO(0.f));
      v.store(v_merged + pos * out_stride_n + head_idx * out_stride_h + tx * vec_size);
      if (s_merged != nullptr) {
        s_merged[pos * num_heads + head_idx] = -math::inf;
      }
      continue;
...
#pragma unroll 4
    ...
    st.o.cast_store(v_merged + pos * out_stride_n + head_idx * out_stride_h + tx * vec_size);
    if (s_merged != nullptr) {
      s_merged[pos * num_heads + head_idx] = st.get_lse();
    }
  }
...
#pragma unroll 1
  for (uint32_t i = cta_id; i < seq_len * num_heads; i += num_ctas) {
    ...
    if (num_index_sets == 0) {
      vec_t<DTypeO, vec_size> v;
      v.fill(DTypeO(0.f));
      v.store(v_sum + pos * out_stride_n + head_idx * out_stride_h + tx * vec_size);
      continue;
    }
...
#pragma unroll
    ...
    v_sum_vec.cast_store(v_sum + pos * out_stride_n + head_idx * out_stride_h + tx * vec_size);
  }
#if (__CUDACC_VER_MAJOR__ >= 12 && defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900))
  asm volatile("griddepcontrol.launch_dependents;");
#endif
...
cudaError_t VariableLengthMergeStates(DTypeIn* v, float* s, IdType* indptr, DTypeO* v_merged,
                                      ...
                                      cudaStream_t stream = nullptr, uint32_t o_stride_n = 0,
                                      uint32_t o_stride_h = 0) {
  int dev_id = 0;
  int num_sms = 0;
...
cudaError_t VariableLengthAttentionSum(DTypeIn* v, IdType* indptr, DTypeO* v_sum,
  ...
  DISPATCH_HEAD_DIM(head_dim, HEAD_DIM, {
    ...
    dim3 nblks(num_sms * num_blocks_per_sm);
    dim3 nthrs(bdx, bdy);
    void* args[] = {&v,       &indptr,     &v_sum,      &max_seq_len,
                    &seq_len, &num_heads, &o_stride_n, &o_stride_h};
    FLASHINFER_CUDA_CALL(
        cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_size));
