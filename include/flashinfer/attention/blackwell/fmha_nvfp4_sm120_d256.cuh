...
void sm120_nvfp4_qkv_online_register_q_stage_kernel(
  ...
  const int load_thread_idx =
      is_load
          ? thread_idx - kSm120Nvfp4FmhaWarpLoad * cutlass::NumThreadsPerWarp
          : 0;
  const bool load_leader = is_load && load_thread_idx == 0;
  const bool first_load_warp =
      is_load && load_thread_idx < cutlass::NumThreadsPerWarp;
...
#pragma unroll
            for (int token_offset = 0; token_offset < kTransposeTokens;
              ...
              uint32_t row_word = 0;
              if (token < kv_len_tokens) {
                row_word = sm120_nvfp4_linear_v_data_cache_word<true>(
...
#pragma unroll
      ...
      for (int idx = load_thread_idx; idx < kCutlassTileM * kCutlassTileK / 16;
        ...
        qk_sSFA_m(row, k0, write_stage) = make_ue4m3_raw(scale);
      }
    }
  };
